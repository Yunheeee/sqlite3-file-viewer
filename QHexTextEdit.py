...
class QCodeEditor(QPlainTextEdit):
    ...
    class NumberBar(QWidget):
        ...
        def paintEvent(self, event):
            ...
            while block.isValid():
                ...
                #str = "%.8d    " % (blockNumber)
                hexs = self.editor.document().findBlockByLineNumber(blockNumber).text()
                hex_list = hexs.split(" ")
        ...
        def find_loc_block(self, y):
            ...
            while block.isValid():
                ...
                if (y >= block_top and y <= (block_top + self.editor.fontMetrics().height())):
                    return block.text()
    ...
    def __init__(self):
        ...
        self.setViewportMargins(self.number_bar.fontMetrics().width('ffffffff    1234567812345678'), 0, 0, 0)
        # 滚动区域周围的边距设置为左侧
    ...
    def resizeEvent(self, *e):
        ...
        '''
        print (cr.height())
        print (self.number_bar.fontMetrics().height())
        print ("lineNum:", cr.height()/self.number_bar.fontMetrics().height())
        '''
    ...
    def highligtCurrentLine(self):
        ...
        if newCurrentLineNumber != self.currentLineNumber:
            ...
            hi_selection.format.setProperty(QTextFormat.FullWidthSelection, False)
            # 设定文本的整个宽度将显示为选中状态

            hi_selection.cursor = self.textCursor()
            hi_selection.cursor.clearSelection()

            #hi_selection.cursor.movePosition(QTextCursor.NextCharacter, QTextCursor.KeepAnchor)
            # 通过将锚点设置为光标位置来清除当前选择
    ...
    def markDataArea(self, color, offinfo):
        start, end = offinfo
        #设定颜色
        lineColor = color
        ses = self.extraSelections()
        ...
        ses.append(selection)
        self.setExtraSelections(ses)
